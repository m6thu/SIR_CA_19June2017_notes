...
## SIS model script for comparing with mass model using simplified calcInfect
...
for(i in 1:iterations){
    ...
    new_infect_grid <- calcInfectSimple(grid_map, prob_infection, 'i1')
...
png("r02_di5_simplified.png")
ggplot(df) + geom_line(aes(x=idx, y=s_n), color='green') + geom_line(aes(x=idx, y=i_n), color='red')
dev.off()

######### Mass model ###########
png("r02_di5_simplified_mass.png")
calcSIStheoretical(beta = R0, mu = 1, t_series = seq(0, 15, 0.001), n = n)
dev.off()
